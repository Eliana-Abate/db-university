...
SELECT `courses`.`name` AS `Materia`, `teachers`.`id` AS `Professore`
FROM `course_teacher`
JOIN `courses`
...
SELECT DISTINCT `teachers`.`surname` AS `Cognome_Professore`, `teachers`.`name` AS `Nome_Professore`, `departments`.`name` AS `Dipartimento`
FROM `teachers`
JOIN `course_teacher`
...
SELECT `students`.`surname`, `students`.`name`, `courses`.`id` AS `Materia`, `exams`.`date` AS `Data_Esame`,`exam_student`.`vote`
FROM `students` 
JOIN `exam_student`
